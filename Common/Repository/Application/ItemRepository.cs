...
namespace Common.Repository.Application
{
    public class ItemRepository : IItemRepository
    {
        ...
        {
            var get = myContext.Item.Where(x => (x.Name.Contains(values) || x.Id.ToString().Contains(values) || x.Brand.Contains(values) || x.Stock.ToString().Contains(values) || x.Price.ToString().Contains(values) || x.Type_Id.ToString().Contains(values)) && x.IsDelete == false).ToList();
            return get;
        }
